...
Registry.prototype = {
    ...
    hash: function (fn1, fn2) {
        var result = 0;
        var name = fn1.name || fn1.wasabiFnName;
        var i;

        if (fn2) {
            var name2 = fn2.name || fn2.wasabiFnName;
            name += '.' + name2;
        }
    ...
    addClass: function (klass, instance) {
        ...
        for (k in klass.prototype) {
            ...
            if (typeof klass.prototype[k] === 'function' && !(/Args$/.test(k)) && /^(rpc|c2s|s2c)/.test(k)) {
                ...
                // use property name if function.name isn't set
                fn.wasabiFnName = fn.name.length ? fn.name : k;  
